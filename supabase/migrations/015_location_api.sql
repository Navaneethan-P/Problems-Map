...
BEGIN
  IF p_type = 'country' THEN
    RETURN QUERY 
      SELECT c.id, c.name, NULL::UUID, ST_AsGeoJSON(ST_Envelope(c.geometry))::JSON
      FROM public.countries c
      ORDER BY c.name;
      
  ELSIF p_type = 'state' THEN
    RETURN QUERY 
      SELECT s.id, s.name, s.country_id, ST_AsGeoJSON(ST_Envelope(s.geometry))::JSON
  ...
  ELSIF p_type = 'district' THEN
    RETURN QUERY 
      SELECT d.id, d.name, d.state_id, ST_AsGeoJSON(ST_Envelope(d.geometry))::JSON
  ...
  ELSIF p_type = 'municipality' THEN
    RETURN QUERY 
      SELECT m.id, m.name, m.district_id, ST_AsGeoJSON(ST_Envelope(m.geometry))::JSON
